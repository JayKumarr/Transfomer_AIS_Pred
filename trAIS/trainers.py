...
def sample(model,
    ...
    for k in range(steps):
        ...
        ix = torch.cat((lat_ix, lon_ix, sog_ix, cog_ix, grid_lat_ix, grid_lon_ix, last_grid_lat_ix, last_grid_lon_ix,
                        gcx_lat_ix, gcx_lon_ix,), dim=-1)
...
class Trainer:
    ...
    def __init__(self, model, train_dataset, test_dataset, config, savedir=None, device=torch.device("cpu"), tb=None,
                 aisdls=None):
        """
        :param model: <TrAISFromer model>
        :param train_dataset: <AISDataset>
        :param test_dataset:  <AISDataset>
        :param config: <config_trAISformer>
        :param savedir: <str> directory path
        :param device: <device> cpu/gpu
        :param tb:
        :param aisdls: dict<AISDataset>
        """
                ...
                with torch.set_grad_enabled(is_train):
                    if return_loss_tuple:
                        logits, loss, loss_tuple = model(seqs,
                                                         ...
                                                         route_class_weight=route_class_weight)
                    else:
